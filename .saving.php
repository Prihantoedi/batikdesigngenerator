<?php
    ...
    $filenameHp = $_SESSION['filename-hp'];
    ...
    if(!empty($user_in_waiting)){ // bila ada user in waiting
        ...
        if(!is_null($fetch_last_id_process)){ // cek dulu kalo ada user sebelumnya dengan status dalam proses
            echo "passing here";
            ...
            if($_SESSION['add-cost-status'] == 0){ // bila tidak ada biaya tambahan, cek apakah pesanan memenuhi batch atau tidak
                ...
                }// batas pengecekkan apabila ada user sebelumnya dengan status dalam proses
                ...
                foreach($user_in_waiting as $user_update){
                    ...
                    if($user_data->num_of_color <= $count_color_used){ // bila jumlah pesanan telah mencapai syarat batch penggunaan warna
                ...
                $sql = mysqli_query($conn, $query) or die(mysqli_error($conn));
                if(!is_null($sql)){ // jika sql tidak empty (ada user sebelumnya dengan status dalam proses)
        ...
        else{ // jika tidak ada user sebelumnya dengan status dalam proses

            // disini harusnya cek juga warna dari user sebelumnya yang masih dalam status tunggu
            // kalo warnanya pas jumlahnya (sesuai batch), maka seluruh user in waiting yang warnanya sama dengan current user diganti statusnya jadi dalam proses
            // dan mengganti durasi pengerjaanya juga

            if($_SESSION['add-cost-status'] != 0){
                // Bila ada biaya tambahan
                // cukup update last_id_process json pada setiap user yang punya status daftar tunggu
                foreach($user_in_waiting as $user){
                    $user_data = json_decode($user);
                ...
                // Bila tidak ada biaya tambahan
                // bila pesanan memenuhi, batch warna maka update status dari setiap user pada waiting list
                ...
                foreach($user_in_waiting as $user_update){
                    ...
                    if($user_data->num_of_color <= $count_color_used){ // bila jumlah pesanan telah mencapai syarat batch penggunaan warna
                        ...
                        if(!is_null($user_data->last_id_process)){
                            //  *** Disini data-data pembeli yang berganti status dalam database akan di update lewat perintah sql ***// 
                            ...
                            $query_update = "UPDATE tbl_hasilbatik SET manufacturing_date  = '$tanggal', status = 'dalam proses' WHERE hasilbatik_id = $user_data->id_order";
                            mysqli_query($conn, $query_update) or die(mysqli_error($conn));
                        }  
            ...
            $sql = mysqli_query($conn, $query) or die(mysqli_error($conn));
            $dataSebelumnya = mysqli_fetch_assoc($sql);
            if(!is_null($dataSebelumnya)){ // jika sql tidak empty (ada user sebelumnya dengan status dalam proses)
                ...
                $tanggalSebelumnya = $dataSebelumnya['manufacturing_date'];
                $durasiSebelumnya = $dataSebelumnya['durasi'];
        ...
        if($process_status == "dalam process") { // cek apakah statusnya dalam proses ?
            ...
            $sql = mysqli_query($conn, $query) or die(mysqli_error($conn));
            $dataSebelumnya = mysqli_fetch_assoc($sql);
            if(!is_null($dataSebelumnya)){ // jika query tidak kosong (ada user sebelumnya dengan status dalam proses)
                // $dataSebelumnya = mysqli_fetch_assoc($sql);

                // if(!is_null($dataSebelumnya)){ // jika ada user sebelumnya dengan status dalam proses
